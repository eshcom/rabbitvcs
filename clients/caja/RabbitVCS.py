#
# This is an extension to the Caja file manager to allow better
# integration with the Subversion source control system.
#
# Copyright (C) 2006-2008 by Jason Field <jason@jasonfield.com>
# Copyright (C) 2007-2008 by Bruce van der Kooij <brucevdkooij@gmail.com>
# Copyright (C) 2008-2008 by Adam Plumb <adamplumb@gmail.com>
# Copyright (C) 2012-2012 by Pavel Antonov <pvantonov@gmail.com>
# Copyright (C) 2014 by Francis Bisson <xezekielx@gmail.com>
#
...
class RabbitVCS(Caja.InfoProvider, Caja.MenuProvider,
				Caja.ColumnProvider, Caja.PropertyPageProvider,
				GObject.GObject):
	"""
	...
	def update_columns(self, item, path, status):
		"""
		Update the columns (attributes) for a given Caja item,
	...
	def get_file_items_full(self, provider, window, items):
		"""
		Menu activated with items selected. Caja also calls this function
	...
	def get_background_items_full(self, provider, window, item):
		...
		@type   window: CajaNavigationWindow
		@param  window:

		@type   item:   CajaVFSFile
		@param  item:
...
class CajaContextMenu(MenuBuilder):
	"""
	Provides a standard Caja context menu (ie. a list of
	"Caja.MenuItem"s).
	"""
	signal = "activate"
...
class CajaMainContextMenu(MainContextMenu):
	def get_menu(self):
		return CajaContextMenu(self.structure, self.conditions, self.callbacks).menu
