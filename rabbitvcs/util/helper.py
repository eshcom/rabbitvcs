...
DT_FORMAT_TIME = "%H:%M"
DT_FORMAT_THISWEEK = "%a %H:%M"
DT_FORMAT_THISYEAR = "%d %b %H:%M"
DT_FORMAT_ALL = "%Y-%m-%d %H:%M"
...
def compare_version(version1, version2):
	...
	for i in range(max(len1, len2)):
		x = int(version1[i]) if i < len1 else 0
		y = int(version2[i]) if i < len2 else 0
		r = x - y
		if r:
			return r
	return 0
...
def format_datetime(dt, format=None):
	...
	if dt.year == 1900:
		...
		if delta.days == 0:
			...
			returner = dt.strftime(DT_FORMAT_THISWEEK)
		# esh: change logic
		elif delta.days >= 7 and dt.year == now.year:
